...
re = requests.get('https://tw.stock.yahoo.com/class-quote?sectorId=46&exchange=TAI')
soup = BeautifulSoup(re.text, 'html.parser')

data = soup.find('time').get('datatime')
# print(data)
...
for row in rows:
    ...
    result.append([data, company, price, status, percentage])

    # print(data, company, price, status, percentage)
print(result)
